...
class DiffusionInAqueousSolution(Application):
    ...
    def create_particles(self):
        ...
        xf = np.arange(0., 1. + self.dx, self.dx)
        xf += -min(xf)
        h = self.hdx * self.dx
        self.h = h
        m = self.dx**self.dim * self.ro
        m_tank = (self.dx)**2 * self.ro
        h_tank = self.hdx * self.dx
        fluid = get_particle_array_fluid(name='fluid', x=xf, y=0., z=0.,
                                         h=h, m=m, rho=self.ro)
        fluid.is_static[:] = 1
        ...
        for i in range(len(xf)):
            if fluid.x[i] >= 0.45 and fluid.x[i] <= 0.55:
                indices_limit.append(i)
    ...
    def create_scheme(self):
        wcsph = FluidsScheme(
            ...
            dim=self.dim, rho0=self.ro, c0=self.c0, pb=self.p0, nu=None,
    ...
    def post_process(self, fname):
        ...
        logfile = os.path.join(
            ...
            'mayoral_2016_4_1_diffusion_in_aqueous_solution_1d.log')
