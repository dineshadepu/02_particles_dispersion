...
class CircularBodyInHSTank(Application):
    def initialize(self):
    ...
    def create_particles(self):
        ...
        # create rigid body
        x, y = create_circle_1(self.rigid_body_length * 2, self.dx)
    ...
    def post_process(self, fname):
        ...
        for sd, rigid_body in iter_output(files, 'rigid_body'):
            ...
            cm_x.append(rigid_body.xcm[0])
            cm_y.append(rigid_body.xcm[1])
            cm_z.append(rigid_body.xcm[2])
            t.append(_t)
        ...
        plt.plot(t, force_z, label="force z")
        plt.legend()
        fig = os.path.join(self.output_dir, "t_vs_xcm.png")
        plt.savefig(fig, dpi=300)


if __name__ == '__main__':
    app = CircularBodyInHSTank()
    app.run()
